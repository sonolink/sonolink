...
class TestCreateNode:
    ...
    def test_create_node_with_inactivity_settings(self, client: Client[MagicMock]) -> None:
    ...
    def test_create_node_uri_and_host_port_conflict(self, client: Client[MagicMock]) -> None:
    ...
    def test_create_node_missing_host_or_port(self, client: Client[MagicMock]) -> None:
        with pytest.raises(ValueError, match="Must specify either uri or host and port"):
            client.create_node(  # pyright: ignore[reportCallIssue]
                host="localhost",
                password="youshallnotpass"
            )
