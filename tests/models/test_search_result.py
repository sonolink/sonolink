...
class TestSearchResultType:
    ...
    def test_is_empty_true_for_empty(
        ...
        assert make_search_result(TrackLoadResult.EMPTY, None).is_empty() is True
    ...
    def test_is_empty_false_for_track(
        ...
        result = make_search_result(TrackLoadResult.TRACK, track_payload())
    ...
    def test_is_error_true_for_error(
        ...
        data = ERROR_PAYLOAD
        assert make_search_result(TrackLoadResult.ERROR, data).is_error() is True
...
class TestSearchResultResult:
    def test_track_result_is_playable(
        ...
        result = make_search_result(
            TrackLoadResult.TRACK, track_payload(title="Only Track")
        )
    ...
    def test_search_result_is_list_of_playables(
        ...
        resolved = make_search_result(TrackLoadResult.SEARCH, payload).result
    ...
    def test_playlist_result_is_playlist(
        ...
        resolved = make_search_result(TrackLoadResult.PLAYLIST, payload).result

        assert isinstance(resolved, Playlist)

    def test_empty_result_is_none(
        self, make_search_result: Callable[..., SearchResult]
    ) -> None:
        assert make_search_result(TrackLoadResult.EMPTY, None).result is None

    def test_error_result_is_none(
        self, make_search_result: Callable[..., SearchResult]
    ) -> None:
        data = ERROR_PAYLOAD
        assert make_search_result(TrackLoadResult.ERROR, data).result is None
...
class TestSearchResultException:
    ...
    def test_exception_populated_on_error(
        self, make_search_result: Callable[..., SearchResult]
    ) -> None:
        data = ERROR_PAYLOAD
        exception = make_search_result(TrackLoadResult.ERROR, data).exception
